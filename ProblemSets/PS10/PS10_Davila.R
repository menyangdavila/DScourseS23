...
nnet_grid   <- grid_regular(penalty(), levels = 10)
nnet_parm_df  <- full_join(nnet_parm_df1,nnet_grid,by=character())

# Workflow
nnet_rec_wf <- workflow() %>%
  add_model(tune_nnet_spec) %>%
  add_formula(high.earner ~ education + marital.status + race + workclass + occupation + relationship + sex + age + capital.gain + capital.loss + hours)

# Tuning results
nnet_rec_res <- nnet_rec_wf %>%
  tune_grid(
    resamples = rec_folds,
    grid = nnet_parm_df
  )
...
# what is the best value of hyperparameters set?
nnet_top_acc  <- show_best(nnet_rec_res, metric = "accuracy")
nnet_best_acc <- select_best(nnet_rec_res, metric = "accuracy")
final_nnet <- finalize_workflow(nnet_rec_wf,
                                nnet_best_acc
)
...
# define a grid over which to try different values of the regularization parameter (number of neighbors)
...
final_svm <- finalize_workflow(svm_rec_wf,
                               svm_best_acc
)
print('*********** svm Model **************')
